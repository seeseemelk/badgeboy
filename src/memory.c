...
static unsigned char* ram_banks; //[0x8000] = {0}; // Max 4 ram banks (only 2 bits to change it), RAM can be 2KB, 8KB or 32KB (in the form of 4 8KB banks)
...
void insert_cartridge(const char* filename) {
    if (rom == NULL) {
        puts("rom is NULL");
        return;
    }
...
void insert_cartridge_raw(unsigned char* cartridge) {
    memcpy(rom, cartridge, 0x200000);
    cartridge_loaded = 1;
}

void insert_cartridge_now() {
    cartridge_loaded = 1;
}
